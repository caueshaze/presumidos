...
pub async fn draft(token: String, id: String) -> Result<BuilderDraft, ServerFnError> {
    ...
    let rows:Vec<(String,String,String,String,i64,Option<String>)>=sqlx::query_as("SELECT pi.id,pi.title,pi.lock_at,pi.reveal_at,pi.sort_order,q.correct_option_id FROM prediction_items pi JOIN custom_questions q ON q.item_id=pi.id WHERE pi.event_id=?1 ORDER BY pi.sort_order,pi.id").bind(&id).fetch_all(crate::db::pool()).await.map_err(|e|crate::security::internal_error("event_draft_items",e))?;
    let mut items = Vec::with_capacity(rows.len());
    for (id, title, lock_at, reveal_at, sort_order, correct_option_id) in rows {
...
pub async fn publish(token: String, event_id: String, csrf: String) -> Result<(), ServerFnError> {
    ...
    let invalid:Option<(String,)>=sqlx::query_as("SELECT pi.title FROM prediction_items pi LEFT JOIN custom_question_options o ON o.item_id=pi.id WHERE pi.event_id=?1 GROUP BY pi.id HAVING COUNT(o.id)<2 LIMIT 1").bind(&event_id).fetch_optional(db).await.map_err(|e|crate::security::internal_error("event_publish_validate",e))?;
