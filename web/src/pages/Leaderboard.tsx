...
export function LeaderboardPage() {
  ...
  const [selectedPool, setSelectedPool] = useState(routePoolId ?? searchParams.get("poolId") ?? "");
                ...
                <div className="grid grid-cols-3 gap-3">
                  {podium.map((entry, i) => (
                    ...
                    >
                      ...
                      </div> : <div className="text-xs text-ink-muted">✓ {entry.correctResults} {entry.correctResults === 1 ? "acerto" : "acertos"}</div>}
                    </motion.div>
                  ))}
                </div>
