  ...
  return <PageShell className="py-9 sm:py-12">
    <header className="max-w-2xl">
      ...
      <div className="mt-7 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
        <Button size="lg" aria-pressed={mode === "create"} onClick={() => openMode("create")} className={cn("w-full justify-center sm:w-auto", mode === "create" && "ring-2 ring-mint-dark/35 ring-offset-2 ring-offset-bg")}><Plus className="h-4 w-4" /> Criar bolão</Button>
        <Button variant="outline" size="lg" aria-pressed={mode === "join"} onClick={() => openMode("join")} className={cn("w-full justify-center sm:w-auto", mode === "join" && "ring-2 ring-mint-dark/30 ring-offset-2 ring-offset-bg")}><Ticket className="h-4 w-4" /> Entrar com código</Button>
      </div>
    </header>
    ...
    <section className="mt-12" aria-labelledby="active-pools-title">
      ...
      {pools.isLoading ? <div className="mt-4"><LoadingState label="Carregando bolões em andamento..." /></div> : pools.isError ? <div className="mt-4"><ErrorState onRetry={() => void pools.refetch()}>{(pools.error as Error).message}</ErrorState></div> : activePools.length === 0 ? <div className="mt-4"><EmptyState title="Nenhum bolão em andamento agora.">Crie um bolão ou entre com um código para começar.</EmptyState></div> : <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{activePools.map((summary, index) => <ActivePoolCard key={summary.pool.id} summary={summary} index={index} onOpen={() => navigate(`/pools/${summary.pool.id}`)} />)}</div>}
    </section>
  </PageShell>;
}
