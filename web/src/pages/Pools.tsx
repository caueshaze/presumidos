...
export function PoolsPage() {
  ...
  return <PageShell>
    <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl">Meus bolões</h1><p className="mt-1 text-ink-muted">Acompanhe seus palpites, resultados e a disputa com a galera.</p></div><div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-[190px]"><Button className="w-full justify-center" onClick={() => navigate("/dashboard?mode=create")}><Plus className="h-4 w-4" /> Criar bolão</Button><Button className="w-full justify-center" variant="outline" onClick={() => navigate("/dashboard?mode=join")}><Ticket className="h-4 w-4" /> Entrar com código</Button><Button className="w-full justify-center sm:w-auto" variant="link" size="sm" onClick={() => navigate("/events/new")}>Criar evento</Button></div></header>
